# TLX Bug-Bounty Workspace — Codex Instructions
...
1. When the user starts work on a target, look for `targets/<name>/http.md`.
   ...
   `.Codex/skills/target-init/SKILL.md`).
...
## MCP servers (pre-wired in `.Codex/settings.json`)
...
7. **External API keys — restricted whitelist.** `ANTHROPIC_API_KEY`
   ...
   compaction, summarization — runs as **Codex (this
   conversation)**. If a task cannot be done from Codex, say
...
- Each skill returns a structured result block; skill bodies define the
  ...
  skill's key. This is how Codex resumes a half-done target on a new
  session.
...
The personal global memory at `~/.Codex/projects/-Users-soural-...` is
