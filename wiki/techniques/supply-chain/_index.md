# techniques · supply-chain

_Auto-generated index._

## Pages

- [dependency confusion](dependency-confusion.md)
- [electron asar credential leak](electron-asar-credential-leak.md)
- [git config worktree fallback rce](git-config-worktree-fallback-rce.md)
- [maintainer domain takeover](maintainer-domain-takeover.md)
- [npm cache poisoning 404](npm-cache-poisoning-404.md)
- [npx binary package confusion](npx-binary-package-confusion.md)
- [zip slip path traversal](zip-slip-path-traversal.md)
