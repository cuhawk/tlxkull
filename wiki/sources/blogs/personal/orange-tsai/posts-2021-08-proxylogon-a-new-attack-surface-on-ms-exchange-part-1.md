...
title: "A New Attack Surface on MS Exchange Part 1 - ProxyLogon! | Orange Tsai"
author: "Orange Tsai"
published: 2021-08-05T16:00:00.000Z
description: "The series of A New Attack Surface on MS Exchange: A New Attack Surface on MS Exchange Part 1 - ProxyLogon! A New Attack Surface on MS Exchange Part 2 - ProxyOracle! A New Attack Surface on MS Excha"
---
...
# [Intro](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#Intro "Intro") Intro
...
# [Why targeting on Exchange Server?](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#Why-targeting-on-Exchange-Server "Why targeting on Exchange Server?") Why targeting on Exchange Server?
...
## [The most special](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#The-most-special "The most special") The most special
...
## [The most interesting](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#The-most-interesting "The most interesting") The most interesting
...
## [The most surprising](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#The-most-surprising "The most surprising") The most surprising
...
# [Where is the new attack surface](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#Where-is-the-new-attack-surface "Where is the new attack surface") Where is the new attack surface
...
# [The CAS architecture](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#The-CAS-architecture "The CAS architecture") The CAS architecture
...
## [Frontend Proxy Module](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#Frontend-Proxy-Module "Frontend Proxy Module") Frontend Proxy Module
...
### [Frontend Request Section](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#Frontend-Request-Section "Frontend Request Section") Frontend Request Section
...
### [Frontend Proxy Section](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#Frontend-Proxy-Section "Frontend Proxy Section") Frontend Proxy Section
...
### [Frontend Response Section](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#Frontend-Response-Section "Frontend Response Section") Frontend Response Section
...
## [Backend Rehydration Module](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#Backend-Rehydration-Module "Backend Rehydration Module") Backend Rehydration Module
...
|     |     |
| --- | --- |
| ```<br>1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19<br>20<br>21<br>22<br>23<br>24<br>25<br>26<br>27<br>28<br>29<br>30<br>31<br>32<br>33<br>34<br>35<br>36<br>37<br>38<br>39<br>40<br>41<br>42<br>``` | ```<br>private void OnAuthenticateRequest(object source, EventArgs args) {<br>    if (httpContext.Request.IsAuthenticated) {<br>        this.ProcessRequest(httpContext);<br>    }<br>}<br>private void ProcessRequest(HttpContext httpContext) {<br>    CommonAccessToken token;<br>    if (this.TryGetCommonAccessToken(httpContext, out token)) {<br>        // ...<br>    }<br>}<br>private bool TryGetCommonAccessToken(HttpContext httpContext, out CommonAccessToken token) {<br>    string text = httpContext.Request.Headers["X-CommonAccessToken"];<br>    if (string.IsNullOrEmpty(text)) {<br>        return false;<br>    }<br>        <br>    bool flag;<br>    try {<br>        flag = this.IsTokenSerializationAllowed(httpContext.User.Identity as WindowsIdentity);<br>    } finally {<br>        httpContext.Items["BEValidateCATRightsLatency"] = stopwatch.ElapsedMilliseconds - elapsedMilliseconds;<br>    }<br>    token = CommonAccessToken.Deserialize(text);<br>    httpContext.Items["Item-CommonAccessToken"] = token;<br>    <br>    //...<br>}<br>private bool IsTokenSerializationAllowed(WindowsIdentity windowsIdentity) {<br>   flag2 = LocalServer.AllowsTokenSerializationBy(clientSecurityContext);<br>   return flag2;<br>}<br>private static bool AllowsTokenSerializationBy(ClientSecurityContext clientContext) {<br>    return LocalServer.HasExtendedRightOnServer(clientContext, <br>        WellKnownGuid.TokenSerializationRightGuid);  // ms-Exch-EPI-Token-Serialization<br>}<br>``` |

# [The attack surface](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#The-attack-surface "The attack surface") The attack surface
...
# [The ProxyLogon](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#The-ProxyLogon "The ProxyLogon") The ProxyLogon
...
## [CVE-2021-26855 - Pre-auth SSRF](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#CVE-2021-26855-Pre-auth-SSRF "CVE-2021-26855 - Pre-auth SSRF") CVE-2021-26855 - Pre-auth SSRF
...
|     |     |
| --- | --- |
| ```<br>1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>17<br>18<br>19<br>20<br>21<br>22<br>23<br>24<br>25<br>``` | ```<br>protected virtual Uri GetTargetBackEndServerUrl() {<br>    this.LogElapsedTime("E_TargetBEUrl");<br>    Uri result;<br>    try {<br>        UrlAnchorMailbox urlAnchorMailbox = this.AnchoredRoutingTarget.AnchorMailbox as UrlAnchorMailbox;<br>        if (urlAnchorMailbox != null) {<br>            result = urlAnchorMailbox.Url;<br>        } else {<br>            UriBuilder clientUrlForProxy = this.GetClientUrlForProxy();<br>            clientUrlForProxy.Scheme = Uri.UriSchemeHttps;<br>            clientUrlForProxy.Host = this.AnchoredRoutingTarget.BackEndServer.Fqdn;<br>            clientUrlForProxy.Port = 444;<br>            if (this.AnchoredRoutingTarget.BackEndServer.Version < Server.E15MinVersion) {<br>                this.ProxyToDownLevel = true;<br>                RequestDetailsLoggerBase<RequestDetailsLogger>.SafeAppendGenericInfo(this.Logger, "ProxyToDownLevel", true);<br>                clientUrlForProxy.Port = 443;<br>            }<br>            result = clientUrlForProxy.Uri;<br>        }<br>    }<br>    finally {<br>        this.LogElapsedTime("L_TargetBEUrl");<br>    }<br>    return result;<br>}<br>``` |
...
## [CVE-2021-27065 - Post-auth Arbitrary-File-Write](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#CVE-2021-27065-Post-auth-Arbitrary-File-Write "CVE-2021-27065 - Post-auth Arbitrary-File-Write") CVE-2021-27065 - Post-auth Arbitrary-File-Write
...
|     |     |
| --- | --- |
| ```<br>1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>10<br>11<br>12<br>13<br>14<br>15<br>16<br>``` | ```<br>public override RunResult Run(DataRow input, DataTable dataTable, DataObjectStore store, Type codeBehind, Workflow.UpdateTableDelegate updateTableDelegate) {<br>    DataRow dataRow = dataTable.Rows[0];<br>    string value = (string)input[this.InputVariable];<br>    string path = (string)input[this.OutputFileNameVariable];<br>    RunResult runResult = new RunResult();<br>    try {<br>        runResult.ErrorOccur = true;<br>        using (StreamWriter streamWriter = new StreamWriter(File.Open(path, FileMode.CreateNew)))<br>        {<br>            streamWriter.WriteLine(value);<br>        }<br>        runResult.ErrorOccur = false;<br>    }<br>    <br>    // ...<br>}<br>``` |
...
ProxyLogon! The latest Pre-Auth RCE on Microsoft Exchange Server! - YouTube

Tap to unmute

[ProxyLogon! The latest Pre-Auth RCE on Microsoft Exchange Server!](https://www.youtube.com/watch?v=SvjGMo9aMwE) [Orange Tsai](https://www.youtube.com/channel/UCnweRFxfA-xpTbkb83yfBog)

![thumbnail-image](https://yt3.ggpht.com/66_0On1QgFsRTSAbRerD1jtXFnV80Iub8ygJ5DOaIX5UJysOQeSZ93yin2qAWbIEskgXGsB1vLI=s68-c-k-c0x00ffffff-no-rj)

Orange Tsai3.75K subscribers

[Watch on](https://www.youtube.com/watch?v=SvjGMo9aMwE)

# [Epilogue](https://blog.orange.tw/posts/2021-08-proxylogon-a-new-attack-surface-on-ms-exchange-part-1/\#Epilogue "Epilogue") Epilogue
