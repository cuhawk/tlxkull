...
title: "HTTP/1.1 must die: the desync endgame | PortSwigger Research"
published: 2025-08-06T22:20:00
...
DEF CON 33 - HTTP 1 1 Must Die! The Desync Endgame - James 'albinowax' Kettle - YouTube

Tap to unmute

[DEF CON 33 - HTTP 1 1 Must Die! The Desync Endgame - James 'albinowax' Kettle](https://www.youtube.com/watch?v=PUCyExOr3sE) [DEFCONConference](https://www.youtube.com/channel/UC6Om9kAkl32dWlDSNlDS9Iw)

DEFCONConference400K subscribers

## Table of contents

- [The desync endgame](https://portswigger.net/research/http1-must-die#the-desync-endgame)

  - [The fatal flaw in HTTP/1.1](https://portswigger.net/research/http1-must-die#the-fatal-flaw-in-http11)
  - [Mitigations that hide but don't fix](https://portswigger.net/research/http1-must-die#mitigations-that-hide-but-dont-fix)
  - [Hacking 20 million websites by accident](https://portswigger.net/research/http1-must-die#hacking-20-million-websites-by-accident)
  - ["HTTP/1 is simple" and other lies](https://portswigger.net/research/http1-must-die#http1-is-simple-and-other-lies)

- [A strategy to win the desync endgame](https://portswigger.net/research/http1-must-die#winning-the-desync-endgame)

  - [Detecting parser discrepancies](https://portswigger.net/research/http1-must-die#detecting-parser-discrepancies)
  - [Understanding V-H and H-V discrepancies](https://portswigger.net/research/http1-must-die#understanding-v-h-and-h-v-scenarios)
  - [Turning a V-H discrepancy into a CL.0 desync](https://portswigger.net/research/http1-must-die#turning-v-h-into-a-cl.0-desync)
  - [Exploiting H-V on IIS behind ALB](https://portswigger.net/research/http1-must-die#exploiting-h-v-on-iis-behind-alb)
  - [Exploiting H-V without Transfer-Encoding](https://portswigger.net/research/http1-must-die#exploiting-h-v-without-transfer-encoding)

- [0.CL desync attacks](https://portswigger.net/research/http1-must-die#0.cl-desync-attacks)

  - [The 0.CL deadlock](https://portswigger.net/research/http1-must-die#the-0.cl-deadlock)
  - [Moving beyond 400 Bad Request](https://portswigger.net/research/http1-must-die#moving-beyond-400-bad-request)
  - [Converting 0.CL into CL.0 with a double-desync](https://portswigger.net/research/http1-must-die#converting-0.cl-into-cl.0-with-a-double-desync)
  - [More desync attacks are coming](https://portswigger.net/research/http1-must-die#more-desync-attack-are-coming)

- [Expect-based desync attacks](https://portswigger.net/research/http1-must-die#expect-based-desync-attacks)

  - [Bypassing response header removal](https://portswigger.net/research/http1-must-die#bypassing-response-header-removal)
  - [0.CL desync via vanilla Expect - T-Mobile](https://portswigger.net/research/http1-must-die#0.cl-desync-via-vanilla-expect---t-mobile)
  - [0.CL desync via obfuscated Expect - Gitlab](https://portswigger.net/research/http1-must-die#0.cl-desync-via-obfuscated-expect---gitlab)
  - [CL.0 desync via vanilla Expect - Netlify CDN](https://portswigger.net/research/http1-must-die#cl.0-desync-via-vanilla-expect---netlify-cdn)
  - [CL.0 desync via obfuscated Expect - Akamai CDN](https://portswigger.net/research/http1-must-die#cl.0-desync-via-obfuscated-expect---akamai-cdn)

- [Defending against HTTP desync attacks](https://portswigger.net/research/http1-must-die#defending-against-http-desync-attacks)

  - [Why patching HTTP/1.1 is not enough](https://portswigger.net/research/http1-must-die#why-patching-http11-is-not-enough)
  - [How secure is HTTP/2 compared to HTTP/1?](https://portswigger.net/research/http1-must-die#how-secure-is-http2-compared-to-http1)
  - [How to survive with HTTP/1.1](https://portswigger.net/research/http1-must-die#how-to-cope-with-http1)
  - [How you can help kill HTTP/1.1](https://portswigger.net/research/http1-must-die#help-kill-http1)

- [Conclusion](https://portswigger.net/research/http1-must-die#conclusion)
...
Host: psres.net
X: y``HTTP/2 200 OK
Cf-Cache-Status: HIT``GET / HTTP/2
Host: <redacted>
...
GET /404 HTTP/1.1
X: y``HTTP/1.1 200 OK``GET / HTTP/1.1
...
Host: <vpn.redacted>````Host: x/x``HTTP/1.1 400 Bad Request``Xost: x/x``HTTP/1.1 412 Precondition Failed`` Host: x/x``HTTP/1.1 200 OK`` Xost: x/x``HTTP/1.1 412 Precondition Failed`
...
Content-length:
92
``HTTP/1.1 200 OK``GET /z HTTP/1.1
Content-Length: 180
Foo: GET /y HTTP/1.1
...
X: Y``HTTP/1.1 200 OK
Location: /Logon?returnUrl=/bb``GET / HTTP/1.1
Host: <redacted>
``HTTP/1.1 200 OK
...
``HTTP/1.1 404 Not Found``GET /logout HTTP/1.1
Host: <redacted>.t-mobile.com
Content-Length: 100
...
``HTTP/1.1 404 Not Found``POST /authenticate HTTP/1.1
Host: ???
...
Host: www.sky.com
X: X
``HTTP/1.1 404 Not Found``GET /anything HTTP/1.1
Host: auth.lastpass.com
...
[**The Fragile Lock:**\\
Novel Bypasses For SAML Authentication 10 December 2025The Fragile Lock:Novel Bypasses For SAML Authentication](https://portswigger.net/research/the-fragile-lock) [**How to distinguish HTTP pipelining from request smuggling** 19 August 2025How to distinguish HTTP pipelining from request smuggling](https://portswigger.net/research/how-to-distinguish-http-pipelining-from-request-smuggling) [**Gotta cache 'em all: bending the rules of web cache exploitation** 08 August 2024Gotta cache 'em all: bending the rules of web cache exploitation](https://portswigger.net/research/gotta-cache-em-all) [**Splitting the email atom: exploiting parsers to bypass access controls** 07 August 2024Splitting the email atom: exploiting parsers to bypass access controls](https://portswigger.net/research/splitting-the-email-atom)