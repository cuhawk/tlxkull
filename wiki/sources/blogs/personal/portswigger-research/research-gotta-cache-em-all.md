...
published: 2024-08-08T22:27:46
description: "Through the years, we have seen many attacks exploiting web caches to hijack sensitive information or store malicious payloads. However, as CDNs became more popular, new discrepancies between propriet"
---

# Gotta cache 'em all: bending the rules of web cache exploitation

![Martin Doyhenard](https://portswigger.net/content/images/logos/portswigger-logo.jpg)

### [Martin Doyhenard](https://portswigger.net/research)
...
![evil keys used to obtain secrets](https://portswigger.net/cms/images/93/82/c528-article-gotta-cache-em-all-image-article.png)
...
In this paper will explore how different HTTP servers and proxies behave when parsing specially crafted URLs and explore ambiguities in the RFC that lead to path confusion. It will also introduce a set of novel techniques that can be used to leverage parser discrepancies and achieve arbitrary [web cache poisoning](https://portswigger.net/web-security/web-cache-poisoning) and deception in countless websites and CDN providers.

This research is also available in [print/download-friendly PDF](https://portswigger.net/kb/papers/kapvrid/gotta-cache-em-all.pdf) format

You can also watch the recording of my DEFCON presentation here:

DEF CON 32 - Gotta Cache ‘em all bending the rules of web cache exploitation - Martin Doyhenard - YouTube

Tap to unmute

[DEF CON 32 - Gotta Cache ‘em all bending the rules of web cache exploitation - Martin Doyhenard](https://www.youtube.com/watch?v=70yyOMFylUA) [DEFCONConference](https://www.youtube.com/channel/UC6Om9kAkl32dWlDSNlDS9Iw)

DEFCONConference400K subscribers

## Outline

- [Background](https://portswigger.net/research/gotta-cache-em-all#background)

  - [Web caches](https://portswigger.net/research/gotta-cache-em-all#webcaches)
  - [Poisoning and deception](https://portswigger.net/research/gotta-cache-em-all#poisondeception)

- [URL discrepancies](https://portswigger.net/research/gotta-cache-em-all#urldiscrepancies)

  - [Delimiters](https://portswigger.net/research/gotta-cache-em-all#delimiters)
  - [Normalization](https://portswigger.net/research/gotta-cache-em-all#normalization)

- [Arbitrary Web Cache Deception](https://portswigger.net/research/gotta-cache-em-all#cachedeception)

  - [Limitations](https://portswigger.net/research/gotta-cache-em-all#limitations)
  - [Static extensions](https://portswigger.net/research/gotta-cache-em-all#extensions)
  - [Static directories](https://portswigger.net/research/gotta-cache-em-all#directories)
  - [Static files](https://portswigger.net/research/gotta-cache-em-all#files)

- [Arbitrary Web Cache Poisoning](https://portswigger.net/research/gotta-cache-em-all#cachepoisoning)

  - [Key normalization](https://portswigger.net/research/gotta-cache-em-all#keynormalization)
  - [Exploiting back-end delimiters](https://portswigger.net/research/gotta-cache-em-all#backdelimiters)
  - [Exploiting front-end delimiters](https://portswigger.net/research/gotta-cache-em-all#frontdelimiters)

- [Cache-What-Where](https://portswigger.net/research/gotta-cache-em-all#whatwhere)
- [Defence](https://portswigger.net/research/gotta-cache-em-all#defence)
...
![](https://portswigger.net/cms/images/aa/91/4d39-article-noramlization_table.png)
...
![](https://portswigger.net/cms/images/18/8a/d91c-article-cloudflare_extensions.png)
...
![](https://portswigger.net/cms/images/bd/df/4f4a-article-table1.png)

You can also use the same technique with an encoded character or string. This is useful when the origin server decodes a delimiter before parsing the URL, or if the path is rewritten by the cache before forwarding the request.
...
![](https://portswigger.net/cms/images/99/4f/d7c9-article-table2.png)
...
![](https://portswigger.net/cms/images/df/db/b465-article-table3.png)
...
![](https://portswigger.net/cms/images/40/6b/2a8b-article-table4.png)
...
![](https://portswigger.net/cms/images/5b/a8/7b46-article-table5.png)
...
![](https://portswigger.net/cms/images/f2/24/b888-article-table6.png)
...
![](https://portswigger.net/cms/images/33/27/0564-article-table7.png)
...
In classic web cache poisoning, the attacker attempts to store a malicious response using a URL key that is requested by users while they navigate the vulnerable website. The more frequently the path is visited, the more victims will be affected by the malicious payload. You can read more about finding web cache poisoning vulnerabilities in James Kettle's research [Practical Web Cache Poisoning](https://portswigger.net/kb/papers/7q1e9u9a/web-cache-poisoning.pdf) and [Web Cache Entanglements: Novel pathways to poisoning](https://portswigger.net/kb/papers/c3wwniai/web-cache-entanglement.pdf).
...
When the origin server uses a special mapping or doesn't normalize the path before generating the response, it's possible to control the key used for stored resources.
An classic example of this are applications that have a self-reflected XSS when an non-existing endpoint is visited.
...
GET /<Backend\_Path><Path\_Traversal><Poisoned\_Path>

![](https://portswigger.net/cms/images/38/e2/6160-article-table8.png)

The double dot-segment is used in this example as the payload already contains a slash. Adjust the path traversal to resolve to the desired poisoned endpoint. The same technique can be applied if a special mapping is used for the backend\_path placeholder.
...
GET /<Backend\_Path><Delimiter><Path\_Traversal><Poisoned\_Path>

![](https://portswigger.net/cms/images/7c/82/e482-article-table9.png)
...
![](https://portswigger.net/cms/images/d3/d9/9cf7-article-hash_delimiter.png)
...
![](https://portswigger.net/cms/images/8d/e8/23e7-article-table12.png)
...
![](https://portswigger.net/cms/images/7b/60/6180-article-table14.png)
...
[Black Hat](https://portswigger.net/research/black-hat) [Martin Favourites](https://portswigger.net/research/martin-doyhenard) [web cache poisoning](https://portswigger.net/research/web-cache-poisoning) [web cache deception](https://portswigger.net/research/web-cache-deception)
...
[**The Fragile Lock:**\\
Novel Bypasses For SAML Authentication 10 December 2025The Fragile Lock:Novel Bypasses For SAML Authentication](https://portswigger.net/research/the-fragile-lock) [06 August 2025](https://portswigger.net/research/http1-must-die) [**Splitting the email atom: exploiting parsers to bypass access controls** 07 August 2024Splitting the email atom: exploiting parsers to bypass access controls](https://portswigger.net/research/splitting-the-email-atom) [**Listen to the whispers: web timing attacks that actually work** 07 August 2024Listen to the whispers: web timing attacks that actually work](https://portswigger.net/research/listen-to-the-whispers-web-timing-attacks-that-actually-work)