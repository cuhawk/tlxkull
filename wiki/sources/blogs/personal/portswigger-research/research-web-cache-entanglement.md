...
source_url: https://portswigger.net/research/web-cache-entanglement
title: "Web Cache Entanglement: Novel Pathways to Poisoning | PortSwigger Research"
published: 2020-08-05T18:30:00
description: "Caches are woven into websites throughout the net, discreetly juggling data between users, and yet they are rarely scrutinized in any depth. In this paper, I'll show you how to remotely probe through"
---
...
Web Cache Entanglement: Novel Pathways to Poisoning - James Kettle (albinowax) - YouTube

Tap to unmute

[Web Cache Entanglement: Novel Pathways to Poisoning - James Kettle (albinowax)](https://www.youtube.com/watch?v=bDxYWGxuVqE) [PortSwigger](https://www.youtube.com/channel/UCkytgKNbJ0L1UuN1K27GAKA)
...
So far, we've seen that when sites exclude the entire query string from the cache key, quite a few attacks become possible. But what if a site is simply excluding a specific parameter - say, a harmless analytics parameter like utm\_content? In theory, this is unexploitable as long as the site doesn't have any gadgets that reflect the entire URL.
...
This parsing quirk has numerous security implications, and one of them is highly relevant. On a system configured to exclude utm\_content from the cache key, the following two requests are identical as they only have one keyed parameter - callback.
...
However, Rails sees three parameters - callback, utm\_content, and callback. It prioritizes the second callback value, giving us full control over it.
...
There's no issue with the proxy\_cache\_key setting here; in fact, it's very similar to nginx's default cache key. But if you look at [nginx's documentation for proxy\_pass](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_pass), you'll find a clue to the problem:
