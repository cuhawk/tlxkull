...
source_url: https://portswigger.net/research/trace-desync-attack
title: "Making desync attacks easy with TRACE | PortSwigger Research"
published: 2024-03-19T14:00:00
description: "Have you ever found an HTTP desync vulnerability that seemed impossible to exploit due to its complicated constraints? In this blogpost we will explore a new exploitation technique that can be used to"
---
...
![picture of a hidden bottle with poison to represent smuggled attack](https://portswigger.net/cms/images/5e/4b/2606-article-article_img.png)
...
I recently came across an HTTP/2 Desync vulnerability (a.k.a [HTTP request smuggling](https://portswigger.net/web-security/request-smuggling)) in a Bug Bounty program that had some HTTP/2 header injection issues. Specifically, it was possible to inject a line break character in a header’s value, letting me smuggle transfer-encoding header which would eventually split the request at backend.
...
But what’s even more interesting is that we can use the TRACE response to build a payload to completely compromise the application, by combining it with Response Smuggling and [Web Cache Poisoning](https://portswigger.net/web-security/web-cache-poisoning). Let's see how:
...
[Trace](https://portswigger.net/research/trace) [Request Smuggling](https://portswigger.net/research/request-smuggling) [Web Caching](https://portswigger.net/research/web-caching) [Martin Favourites](https://portswigger.net/research/martin-doyhenard)

[Back to all articles](https://portswigger.net/research/articles)

## Related Research

[**How to distinguish HTTP pipelining from request smuggling** 19 August 2025How to distinguish HTTP pipelining from request smuggling](https://portswigger.net/research/how-to-distinguish-http-pipelining-from-request-smuggling) [06 August 2025](https://portswigger.net/research/http1-must-die) [**Gotta cache 'em all: bending the rules of web cache exploitation** 08 August 2024Gotta cache 'em all: bending the rules of web cache exploitation](https://portswigger.net/research/gotta-cache-em-all) [**Making HTTP header injection critical via response queue poisoning** 22 September 2022Making HTTP header injection critical via response queue poisoning](https://portswigger.net/research/making-http-header-injection-critical-via-response-queue-poisoning)