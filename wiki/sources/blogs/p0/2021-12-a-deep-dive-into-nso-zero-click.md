---
source: p0
source_url: https://projectzero.google/2021/12/a-deep-dive-into-nso-zero-click.html
title: "A deep dive into an NSO zero-click iMessage exploit: Remote Code Execution - Project Zero"
description: "Posted by Ian Beer & Samuel Groß of Google Project Zero We want to thank Citizen Lab ..."
---
...
![](https://projectzero.google/images/oldblog/blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhvP0V_s870_image2%281%29.jpg)
...
The ImageIO library, [as detailed in a previous Project Zero blogpost](https://projectzero.google/2020/04/fuzzing-imageio.html), is used to guess the correct format of the source file and parse it, completely ignoring the file extension. Using this "fake gif" trick, over 20 image codecs are suddenly part of the iMessage zero-click attack surface, including some very obscure and complex formats, remotely exposing probably hundreds of thousands of lines of code.
...
![](https://projectzero.google/images/oldblog/blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEi2YXg_s352_image4%284%29.png)
...
![](https://projectzero.google/images/oldblog/blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEj2k3S_s327_image1%285%29.png)
...
![](https://projectzero.google/images/oldblog/blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEh-7Ri_s267_image3%285%29.png)
...
|     |
| --- |
| Guint numSyms; // (1)<br>numSyms = 0;<br>for (i = 0; i < nRefSegs; ++i) {<br>    if ((seg = findSegment(refSegs\[i\]))) {<br>      if (seg->getType() == jbig2SegSymbolDict) {<br>numSyms += ((JBIG2SymbolDict \*)seg)->getSize();  // (2)<br>      } else if (seg->getType() == jbig2SegCodeTable) {<br>        codeTables->append(seg);<br>      }<br>    } else {<br>      error(errSyntaxError, getPos(),<br>            "Invalid segment reference in JBIG2 text region");<br>      delete codeTables;<br>      return;<br>    }<br>}<br>...<br>// get the symbol bitmaps<br>syms = (JBIG2Bitmap \*\*)gmallocn(numSyms, sizeof(JBIG2Bitmap \*)); // (3)<br>kk = 0;<br>for (i = 0; i < nRefSegs; ++i) {<br>    if ((seg = findSegment(refSegs\[i\]))) {<br>      if (seg->getType() == jbig2SegSymbolDict) {<br>        symbolDict = (JBIG2SymbolDict \*)seg;<br>        for (k = 0; k < symbolDict->getSize(); ++k) {<br>syms\[kk++\] = symbolDict->getBitmap(k); // (4)<br>        }<br>      }<br>    }<br>} |
...
![](https://projectzero.google/images/oldblog/blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEj9fFs_s506_image6%282%29.png)
...
![](https://projectzero.google/images/oldblog/blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEj0hda_s1070_image7%282%29.png)
...
![](https://projectzero.google/images/oldblog/blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEidgMV_s550_image5%282%29.png)
...
![](https://projectzero.google/images/oldblog/blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjFkcS_s551_image9%281%29.png)
...
![](https://projectzero.google/images/oldblog/blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjBpKF_s265_image8%282%29.png)
