---
source: bughunters
source_url: https://bughunters.google.com/blog/llvm-cfi-and-cross-language-llvm-cfi-support-for-rust
title: "LLVM CFI and Cross-Language LLVM CFI Support for Rust - Google Bug Hunters"
description: "We’re pleased to share that we’ve worked with the Rust community to add LLVM CFI and cross-language LLVM CFI (and LLVM KCFI and cross-language LLVM KCFI) to the Rust compiler as part of our work in the Rust Exploit Mitigations Project Group. Check out details in this post!"
---

[Skip to Content (Press Enter)](https://bughunters.google.com/blog/llvm-cfi-and-cross-language-llvm-cfi-support-for-rust#main-content)
...
![](https://storage.googleapis.com/bughunters-article-images/blogs/rcvalle.jpg)
...
We’re pleased to share that we’ve worked with the Rust community to add LLVM
CFI and cross-language LLVM CFI (and LLVM KCFI and cross-language LLVM KCFI) to
the Rust compiler as part of our work in the [Rust Exploit Mitigations Project\\
Group](https://www.rust-lang.org/governance/teams/compiler#Exploit%20Mitigations%20Project%20Group).
This is the first cross-language, fine-grained, forward-edge control flow
protection implementation for mixed-language binaries that we know of.
...
memory corruption and concurrency issues.\[1\] Therefore, it is generally
accepted that linking foreign C- or C++-compiled code into a program written in
Rust may degrade the security of the program.
...
(See the _Control flow protection_ section.) This was later
...
The following Table I summarizes interoperability-related risks when building
programs for the Linux operating system on the AMD64 architecture and equivalent
without forward-edge control flow protection support in the Rust compiler.

|  | Without using Unsafe Rust | Using Unsafe Rust |
| --- | --- | --- |
| Rust-compiled code only | ▼I Indirect branches in Rust-compiled code are not validated. \[2\] | ▼ Unsafe Rust is susceptible to memory corruption and concurrency issues. <br>▼ Indirect branches in Rust-compiled code are not validated. |
...
| Linking foreign Rust -compiled code into a program written in C or C++ \[3\] | ▲II Indirect branches in C- and C++-compiled code are validated. <br>▼ C and C++ are susceptible to memory corruption and concurrency issues. <br>▼ Indirect branches in Rust-compiled code are not validated. | ▲ Indirect branches in C- and C++-compiled code are validated. <br>▼ C and C++ are susceptible to memory corruption and concurrency issues. <br>▼ Unsafe Rust is susceptible to memory corruption and concurrency issues. <br>▼ Indirect branches in Rust-compiled code are not validated. |
...
when migrating to Rust by gradually replacing C or C++ with Rust, and when C or
C++ and Rust-compiled code share the same virtual address space.
...
There are several details in designing and implementing fine-grained
cross-language forward-edge control flow protection using function pointer type
...
_Fig. 1. Example extern "C" function using Rust integer type._
...
already resolved to their respective `ty::Ty` type representations (i.e., their
respective Rust aliased types) making it currently impossible to identify C
...
_Fig. 2. Example extern "C" function using C type alias._
...
_Fig. 3. Example C library using C integer types and Clang encoding._
...
_Fig. 4. Example Rust program using Rust integer types and the Rust compiler_
_encoding._
...
This option enables normalizing integer types as vendor extended types for LLVM
CFI (and cross-language LLVM KCFI) support with other languages that can’t
represent and encode C/C++ integer types.
...
// normalization option is enabled, which currently uses the
// u<length><type-name> vendor extended type encoding for the C integer types.
void
hello_from_c(long arg)
{
    printf("Hello from C!\n");
...
_Fig. 5. Example C library using C integer types and Clang encoding with the_
_integer normalization option enabled._
...
This makes cross-language LLVM CFI (and cross-language LLVM KCFI) work as is
with minimal loss of granularity.\[4\]

### The cfi\\\_encoding attribute

To provide flexibility for the user, we also provide a
`cfi_encoding`
attribute. The `cfi_encoding` attribute allows the user to define
the CFI encoding for user-defined types.
...
_Fig. 6. Example user-defined types using the cfi\_encoding attribute._
...
_Fig. 7. Example Rust program using Rust integer types and the Rust compiler_
_encoding with the cfi\_types crate types._
...
to be used with the Clang
`-fsanitize-cfi-icall-experimental-normalize-integers` option for
cross-language LLVM CFI support, and proper (i.e., non-rustc) LTO (i.e.,
`-Clinker-plugin-lto`).
...
_Fig. 8. Redirecting control flow using an indirect branch/call to an invalid_
_destination (i.e., within the body of the function)._
...
_Fig. 9. Build and execution of Fig. 8 with LLVM CFI disabled._
...
_Fig. 10. Build and execution of Fig. 8 with LLVM CFI enabled._
...
_Fig. 11. Redirecting control flow using an indirect branch/call to a function_
_with a different number of parameters than arguments intended/passed in the_
_call/branch site._
...
_Fig. 12. Build and execution of Fig. 11 with LLVM CFI disabled._
...
_Fig. 13. Build and execution of Fig. 11 with LLVM CFI enabled._
...
_Fig. 14. Redirecting control flow using an indirect branch/call to a function_
_with different return and parameter types than the return type expected and_
_arguments intended/passed at the call/branch site._
...
_Fig. 15. Build and execution of Fig. 14 with LLVM CFI disabled._
...
_Fig. 16. Build and execution of Fig. 14 with LLVM CFI enabled._
...
_Fig. 18. Redirecting control flow using an indirect branch/call to a function_
_with different return and parameter types than the return type expected and_
_arguments intended/passed in the call/branch site, across the FFI boundary._
...
_Fig. 19. Build and execution of Figs. 17–18 with LLVM CFI disabled._
...
_Fig. 20. Build and execution of Figs. 17–18 with LLVM CFI enabled._
...
cross-language, combined hardware-assisted and software-based, forward-edge
control flow protection, such as Microsoft Windows XFG, ARM Pointer
Authentication-based forward-edge control flow protection, and Intel FineIBT.
...
\[1\] Modern C and C++ compilers provide exploit mitigations to
...
\[2\] An attack that successfully allows a Rust-compiled code
...
\[3\] Assuming forward-edge control flow protection is enabled.

\[4\] E.g., [~1% in the Linux\\
