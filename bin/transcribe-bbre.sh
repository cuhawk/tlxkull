#!/usr/bin/env bash
# Transcribe Bug Bounty Reports Explained (BBRE) VTTs with mlx-whisper large-v3-turbo.
# Idempotent: skips episodes already in transcripts/.
# Usage: bin/transcribe-bbre.sh [N]   # N = max episodes this run (default: all)
...
"$(dirname "$0")/finalize-stranded-bbre.sh" >/dev/null 2>&1 || true
