#!/usr/bin/env bash
# Scan audio_tmp/ for stranded transcript outputs (vid.txt + vid.srt) left by
# orphaned worker processes after bash wrapper died. Move them into
# transcripts/<base>.{txt,srt} by mapping vid back to the original VTT basename.
...
for txt in "$AUDIO_DIR"/*.txt; do
  ...
  # Skip if a worker is still actively writing this vid (lock still owned)
  ...
  # Map vid → base by finding matching VTT
  ...
  ts="$(date -u +%FT%TZ)"
  echo "{\"ts\":\"$ts\",\"vid\":\"$vid\",\"base\":\"$base\",\"status\":\"ok\",\"engine\":\"stranded_recover\"}" >> "$LOG"
  echo "[recovered] $base"
done

echo "Finalize pass done."
