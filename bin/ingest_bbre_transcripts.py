"""Ingest Bug Bounty Reports Explained (BBRE) YouTube transcripts into wiki Chroma collection.

Per-chunk metadata:
  source: "bbre_youtube"
  source_uri: wiki://podcasts/bbre/<base>#chunk-N
  ep_no: int | None (rarely meaningful for BBRE — usually None)
  video_id: 11-char yt id
  title: human title (spaces)
  date: YYYY-MM-DD
  asr_quality: "low" | "normal"

Idempotency: deterministic IDs bbre:<base>:chunk-<N>.
If first chunk ID exists in coll, video is skipped.

Chunking: 6000 chars window, 800 overlap (≈1500/200 tokens). Skip <200-char chunks.

Progress log: append-only JSONL at wiki/sources/podcasts/bbre/whisper/_rag_log.jsonl
"""
...
TRANSCRIPTS = REPO / "wiki" / "sources" / "podcasts" / "bbre" / "whisper" / "transcripts"
LOG_PATH = REPO / "wiki" / "sources" / "podcasts" / "bbre" / "whisper" / "_rag_log.jsonl"
COLLECTION = "wiki"
...
async def main() -> None:
    ...
    only_count = os.environ.get("BBRE_INGEST_LIMIT")
    if only_count:
    ...
    for path in files:
        ...
        for i, ch in enumerate(chunks):
            ids.append(f"bbre:{base}:chunk-{i}")
            docs.append(ch)
            metas.append({
                "source": "bbre_youtube",
                "source_uri": f"wiki://podcasts/bbre/{base}#chunk-{i}",
